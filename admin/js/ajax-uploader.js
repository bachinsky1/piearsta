...
(function($){
	...
	var Ajax_upload = function(button, options){
		...
		this.settings = {
			...
			errorMsgInvalidFile: langStrings.getMsg('error_file_type','Incorrect file type!'),
	...
	Ajax_upload.prototype = {
		...
		create_wrapper : function(){
			...
			setTimeout(function(){
				...
				if (button.outerHeight(true) < 21) {
					$height = 21;
				} else {
					$height = button.outerHeight(true);
				}
				
				if (button.outerWidth(true) < 62) {
					$width = 62;
				} else {
		...
		submit : function(){
			...
			//check extension
			if( !this.checkExt( this.get_ext(file) ) ){
				
				this.error = true;
				$('#' + this.settings.img_loader).remove();
				alert(this.settings.errorMsgInvalidFile);
