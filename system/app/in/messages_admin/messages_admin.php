...
class messages_admin extends Module_cms {
	...
	public function __construct() {
		...
		$this->module = new messages_adminData();
		...
		switch ($this->uri->segment(3)) {								
			case "" : 
				...
				$selected_module = getP("filterModule") ? getP("filterModule") : '';
				$this->tpl->assign("mList", $this->module->createModuleList($selected_module, $predefined_filter_values));
		...
		switch ($action) {
			case "moduleTable" : 
				if ($this->cmsUser->haveUserRole("VIEW", $this->getModuleId())) {
					...
					$result = $this->module->showMessagesTable();
					jsonSend($result);
