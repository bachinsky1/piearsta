...
function moduleJs(id, value) {
	ajaxRequest(moduleTable.getRequestUrl() + "js/", "value=" + value + "&mId=" + id, updateModule);
}

function moduleEnableSelectedS(module, value) {
...
function checkFields(type) {
	...
	var data = new Array($('#name').val(), $('#module_id').val());
	ajaxRequest(moduleTable.getRequestUrl() + "checkname/" + idUrl, 'value=' + encodeURIComponent(JSON.stringify(data)) , checkName);   
}
	...
	$('#editForma :input').each(function(n, element) {
		if($(element).attr('type') !== 'button' && $(element).attr('type') !== 'hidden') {
			if($(element).attr('type') === 'checkbox') {
				...
				if ($(element).attr('id').search("value_")) {
					returnArray[$(element).attr('id')] = $(element).val();
				}
				else {
